...
try {
  const filePath = path.join(__dirname, '../asset/proxy.txt');
  const proxies = fs.readFileSync(filePath, 'utf-8').toString().split('\n');
...
try {
  var UAs = fs.readFileSync(path.join(__dirname, '../asset/ua.txt'), 'utf-8').replace(/\r/g, '').split('\n');
} catch(error) {
...
const rotating = spawn('node', ['rotating.js'], { cwd: 'asset' });

rotating.stdout.on('data', (data) => {
  console.log(`stdout: ${data}`);
});

rotating.stderr.on('data', (data) => {
  console.error(`stderr: ${data}`);
});

rotating.on('close', (code) => {
